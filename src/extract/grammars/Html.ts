...
import type { ExtractorContext, ExtractionResult } from '../Extractor.js';
import type { GraphNode, GraphEdge } from '../../types/Extraction.js';
...
class HtmlExtractor extends TreeSitterExtractor {
  constructor() {
    super({ id: 'html', name: 'HTML', extensions: ['html', 'htm'], grammarPkg: 'tree-sitter-html', nodeConfig });
  }
      ...
      try {
        ...
        for (const tag of tagNodes) {
          ...
          nodes.push({
            ...
            kind: 'tag',
            sourceFile: fileId,
            sourceLocation: { file: fileId, line: tag.startPosition.row + 1, column: tag.startPosition.column + 1 },
            confidence: 'EXTRACTED',
          });
          edges.push({
          ...
          const attrs = startTag.children.filter((c): c is Node => c !== null && c.type === 'attribute');
          for (const attr of attrs) {
            ...
            nodes.push({
              ...
              kind: 'attribute',
              sourceFile: fileId,
              sourceLocation: { file: fileId, line: attr.startPosition.row + 1, column: attr.startPosition.column + 1 },
              confidence: 'EXTRACTED',
            });
            edges.push({
        ...
        const scriptNodes = root.descendantsOfType('script_element').filter((s): s is Node => s !== null);
        for (const script of scriptNodes) {
          ...
          nodes.push({
            ...
            kind: 'tag',
            sourceFile: fileId,
            sourceLocation: { file: fileId, line: script.startPosition.row + 1, column: script.startPosition.column + 1 },
            confidence: 'EXTRACTED',
          });
          edges.push({ id: `${fileId}->${id}:has_scriptlet`, source: fileId, target: id, relation: 'has_scriptlet', confidence: 'EXTRACTED', sourceFile: fileId });
        }
...
function extractHtmlFallback(
  ...
  while ((match = tagRegex.exec(source)) !== null) {
    ...
    nodes.push({ id: tagId, label: `<${tagName}>`, kind: 'tag', sourceFile: fileId, sourceLocation: { file: fileId, line, column: 1 }, confidence: 'EXTRACTED' });
    edges.push({ id: `${fileId}->${tagId}:contains`, source: fileId, target: tagId, relation: 'contains', confidence: 'EXTRACTED', sourceFile: fileId });
  }
  return { nodes, edges };
}
