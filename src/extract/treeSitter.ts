...
import { loadLanguage, createParser } from './TreeSitterLoader.js';
...
export class TreeSitterExtractor implements Extractor {
  ...
  async extract(source: string, ctx: ExtractorContext): Promise<ExtractionResult> {
    ...
    const addNode = (kind: 'class' | 'interface' | 'function' | 'method' | 'import', name: string, loc: Point): string => {
    ...
    const addEdge = (source: string, target: string, relation: GraphEdge['relation'], confidence: GraphEdge['confidence'] = 'EXTRACTED'): void => {
      edges.push({
    ...
    const findName = (node: Node): string | undefined => {
      ...
      // fallback to first identifier descendant that is direct child
      const identifiers = (node.descendantsOfType?.('identifier') ?? []).filter((n): n is Node => n !== null);
    ...
    const findCalleeName = (node: Node): string | undefined => {
      ...
      const funcIds = (func.descendantsOfType?.('identifier') ?? []).filter((n): n is Node => n !== null);
    ...
    const walk = (node: Node) => {
      ...
      if (isType(node, cfg.class)) {
        ...
        ensureFileNode();
        if (parent && (parent.kind === 'class' || parent.kind === 'interface')) addEdge(parent.id, id, 'defines');
        ...
        if (callee) {
          ...
          if (!nodes.has(targetId)) {
            // create an inferred target so the edge is not dangling
            nodes.set(targetId, {
              id: targetId,
              label: callee,
              kind: 'function',
              sourceFile: ctx.sourceFile,
              sourceLocation: { file: ctx.sourceFile, line: node.startPosition.row + 1, column: node.startPosition.column + 1 },
              confidence: 'INFERRED',
            });
          }
